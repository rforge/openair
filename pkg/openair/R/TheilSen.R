...
##' start.year). Where C.start is the concentration at the start date
##' and C.end is the concentration at the end date.
##'
...
##' years i.e. year 1 is used as a reference year. Things are slightly
##' different for monthly values e.g.  \code{avg.time = "month"},
##' which will use the total number of months as a basis of the time
##' span and is therefore able to deal with partial years.  There can
##' be slight differences in the %/yr trend estimate therefore,
...
##'   \code{output <- TheilSen(mydata, "nox")}, this output can be used to
