import.ADMS.mop <- function(file=file.choose()
    ...
    , drop.delim=TRUE, add.suffixes = TRUE 
    , ...)
{
...
##############
#added an add.suffixes option to handle this
...
if(is.logical(add.suffixes)==TRUE){
    if(add.suffixes==TRUE){
    ...
    if(length(add.suffixes)>1){
        check.names[(x.1[1]+4): (x.2[1]-1)] <- paste(add.suffixes[1], check.names[(x.1[1]+4): (x.2[1]-1)], sep=".")
        check.names[(x.2[1]+1): length(check.names)] <- paste(add.suffixes[2], check.names[(x.2[1]+1): length(check.names)], sep=".")
    } else {
        warning("Unexpected add.suffixes option, option treated as FALSE\n       [check openair import settings]"
            , call. = FALSE
        )
    }
}
