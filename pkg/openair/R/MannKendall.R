...
##' Sen-Theil slope estimates and uncertainties.  Further modifications have
##' been made to take account of correlated data based on Kunsch (1989).
##'
...
##'   and the slope is expressed as an average units/year change e.g. ppb.
...
##'   \code{output <- MannKendall(mydata, "nox")}, this output can be used to
...
##'
##' The \code{data} component of the \code{MannKendall} output includes two
##'   subsets: \code{main.data}, the monthly data \code{res2} the trend
##'   statistics. For \code{output <- MannKendall(mydata, "nox")}, these can be
